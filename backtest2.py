...
prob_memo = {}
prob_memo_shelf = shelve.open('probs', flag='c') 
...
m = []
leagues = []
with open("bigpoppa.txt", encoding="utf-8") as f:
...
max_score_1 = 117.02956492182405
max_score_2 = -float('inf')
max_score_3 = -float('inf')
sol_1 = None
sol_2 =  None
sol_3 = None
pinn_params_memo = shelve.open('pinn_params', flag='c')
avg_params_memo = shelve.open('avg_params', flag='c')
    ...
    for x in range(5):
        for y in range(5):
            for q in range(5):
                ...
                flip = False
                game_number = 1
                if w * 1000 + x * 100 + y * 10 + q >= 0 and w+x+y+q>=4:  
                    scores = {}
                    team_scores = {}
                    defense_scores = {}  
                    xgs = {}
                    ...
                    for game in m:
                        if game[4] != 'HomeTeam':
                            ...
                            home_g = float(game[6])
                            away_g = float(game[7])
                            home_xg = float(game[8])
                            away_xg = float(game[9])
                            hcap = float(game[58])
                            orig = hcap
                            ...
                            try:
                                pinn_home_cover_odds = float(game[61])
                                pinn_away_cover_odds = float(game[62])
                            except:
                                try:
                                    pinn_home_cover_odds = float(game[59])
                                    pinn_away_cover_odds = float(game[60])
                                except:
                            ...
                            try:
                                pinn_o = float(game[52])
                                pinn_u = float(game[53])
                            except:
                                try:
                                    pinn_o = float(game[50])
                                    pinn_u = float(game[51])
                                except:
                                    pinn_o = o
                                    pinn_u = u
                            try:
                                pinn_home_odds = float(game[35])
                                pinn_draw_odds = float(game[36])
                                pinn_away_odds = float(game[37])
                            except:
                                try:
                                    pinn_home_odds = float(game[26])
                                    pinn_draw_odds = float(game[27])
                                    pinn_away_odds = float(game[28])
                                except:
                            ...
                            if home in team_scores and len(team_scores[home]) >= 8 and away in team_scores and len(team_scores[away]) >= 8:  
                                bets = []
                                ...
                                home_mult = max((team_goal_ratings[home])**(w/4) * (def_ratings[away])**(x/4) * (lethalness[home])**(y/4) * (gk[away])**(q/4), 0.01)
                                away_mult = max((team_goal_ratings[away])**(w/4) * (def_ratings[home])**(x/4) * (lethalness[away])**(y/4) * (gk[home])**(q/4), 0.01)
                                ...
                                z, home_mu, away_mu, p = pinn_params
                                scores = probs(z, home_line*home_mult-z/2, away_line*away_mult-z/2, p)
                                ...
                                if home_mult >= .1 and away_mult >= .1:  
                                    ...
                                    if pick:
                                        ...
                                        if delta > 0:
                                            ...
                                            s = "P"
                                        pick = (pick, home, away, date, delta, s, (home_g, away_g))
                                        picks = insert(pick, picks)
                                                       
                                    if pick2:
                                        delta = res(pick2, home_g, away_g)
                                        if delta > 0:
                                            s = "W"
                                        elif delta < 0:
                                            s = "L"
                                            ...
                                            s = "P"
                                        pick2 = (pick2, home, away, date, delta, s, (home_g, away_g))
                                        dog_picks = insert(pick2, dog_picks)
                             
                                    if pick3:
                                        delta = res(pick3, home_g, away_g)
                                        if delta > 0:
                                            s = "W"
                                        elif delta < 0:
                                            s = "L"
                                            ...
                                            s = "P"
                                        pick3 = (pick3, home, away, date, delta, s, (home_g, away_g))
                                        fav_picks = insert(pick3, fav_picks)
                                                    
                                
                            total_goals[game[4]] = total_goals.setdefault(game[4], 0) + home_g
                            total_goals[game[5]] = total_goals.setdefault(game[5], 0) + away_g
                            total_goals_ag[game[5]] = total_goals_ag.setdefault(game[5], 0) + home_g
                            total_goals_ag[game[4]] = total_goals_ag.setdefault(game[4], 0) + away_g
                            total_xg[game[4]] = total_xg.setdefault(game[4], 0) + home_xg
                            total_xg[game[5]] = total_xg.setdefault(game[5], 0) + away_xg
                            total_xg_ag[game[5]] = total_xg_ag.setdefault(game[5], 0) + home_xg
                            total_xg_ag[game[4]] = total_xg_ag.setdefault(game[4], 0) + away_xg
                            lethalness[home] = total_goals[home] / total_xg[home] 
                            lethalness[away] = total_goals[away] / total_xg[away] 
                            gk[home] = total_goals_ag[home] / total_xg_ag[home]  
                            gk[away] = total_goals_ag[away] / total_xg_ag[away]  
                                 
                                
                            lines[game[4]] = lines.setdefault(game[4], []) + [home_line]
                            trim_list(lines[game[4]])
                            lines[game[5]] = lines.setdefault(game[5], []) + [away_line]
                            trim_list(lines[game[5]])
                            
                            lines_ag[game[4]] = lines_ag.setdefault(game[4], []) + [away_line]
                            trim_list(lines_ag[game[4]])
                            lines_ag[game[5]] = lines_ag.setdefault(game[5], []) + [home_line]
                            trim_list(lines_ag[game[5]])
                            
                            #home_xg *= (lethalness[game[3]] * gk[game[4]]) ** .25
                            #away_xg *= (lethalness[game[4]] * gk[game[3]]) ** .25
                            xgs[game[4]] = xgs.setdefault(game[4], []) + [home_xg]
                            trim_list(xgs[game[4]])
                            xgs[game[5]] = xgs.setdefault(game[5], []) + [away_xg]
                            trim_list(xgs[game[5]])
                            
                            xgs_ag[game[4]] = xgs_ag.setdefault(game[4], []) + [away_xg]
                            trim_list(xgs_ag[game[4]])
                            xgs_ag[game[5]] = xgs_ag.setdefault(game[5], []) + [home_xg]
                            trim_list(xgs_ag[game[5]])
                            
                            home_goal_score = home_xg/home_line
                            away_goal_score = away_xg/away_line
                            if home_goal_score > 5:
                                print(home_xg, avg_home_line)
                                print('home', home, away, date)
                                print('sddffdf')
                                home_goal_score = 1
                            if away_goal_score > 5:
                                print(away_xg, avg_away_line)
                                print('away', home, away, date)
                                print('sfssfsdf')
                                away_goal_score = 1
                            away_def_score = home_goal_score
                            home_def_score = away_goal_score
                    
                            team_scores[game[4]] = team_scores.setdefault(game[4], []) + [home_goal_score]
                            trim_list(team_scores[game[4]])
                            team_scores[game[5]] = team_scores.setdefault(game[5], []) + [away_goal_score]
                            trim_list(team_scores[game[5]])
                            defense_scores[game[4]] = defense_scores.setdefault(game[4], []) + [home_def_score]
                            trim_list(defense_scores[game[4]])
                            defense_scores[game[5]] = defense_scores.setdefault(game[5], []) + [away_def_score]
                            trim_list(defense_scores[game[5]])
                            
                            team_goal_ratings[home] = weighted_total(xgs[home]) / weighted_total(lines[home])#weighted_geomean(team_scores[key])#sum(team_scores[key]) / len(team_scores[key])
                            team_goal_ratings[away] = weighted_total(xgs[away]) / weighted_total(lines[away])#weighted_geomean(team_scores[key])#sum(team_scores[key]) / len(team_scores[key])
                            def_ratings[home] = weighted_total(xgs_ag[home]) / weighted_total(lines_ag[home])#weighted_geomean(defense_scores[key])#sum(defense_scores[key]) / len(defense_scores[key])
                            def_ratings[away] = weighted_total(xgs_ag[away]) / weighted_total(lines_ag[away])#weighted_geomean(defense_scores[key])#sum(defense_scores[key]) / len(defense_scores[key])    
                            ...
                            game_number += 1
                            avg_time = (time.time()-start_time )/game_number       
                    name =  str(w) + ' ' + str(x) + ' ' + str(y) + ' ' + str(q)
                    
                    profits = []
                    for pick in dog_picks:         
                        profits.append(pick[4])
                    score = maxSubArray(profits)
                    if score > max_score_1 * .8:
                        f = open(name+" dog.txt", "w")
                        for pick in dog_picks:
                            for part in pick[0]:
                                f.write(str(part)+'\t')
                        ...
                        sol_lite = (w, x, y, q)
                        print(score, sol_lite)
                        g = open("doggybag.txt", "a")
                        g.write(str(score)+" ")
                        for num in sol_lite:
                            g.write(str(num))
                        g.write('\n')
                    ...
                    profits = []
                    for pick in fav_picks:
                        profits.append(pick[4])
                    score = maxSubArray(profits)
                    if score > max_score_2 * .8:
                        f = open(name+" fav.txt", "a")
                        for pick in fav_picks:
                            for part in pick[0]:
                                f.write(str(part)+'\t')
                            for i in range(1,len(pick)):
